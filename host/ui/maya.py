...
class MainWindow:
    ...
    def show(self):
        ...
        cmds.menuItem(label="Show Code", parent=menu, command=partial(self._showSelectedModuleCode))
    ...
    def _showSelectedModuleCode(self, *args):
        selIdx = cmds.textScrollList(self.moduleListCtrl, query=True, selectIndexedItem=True)
        if selIdx and selIdx[0] <= len(self.filteredModules):
            spec = self.filteredModules[selIdx[0] - 1][1]
            if spec:
                code = f"import rigBuilder.host.ui.maya as rb_maya\nrb_maya.ModuleWindow(r'{spec}').show()"
                _showCodeDialog(code, title="Rig Builder — Code")

    # Alias for backward compatibility
    _copySelectedModuleCode = _showSelectedModuleCode


def _showCodeDialog(code: str, title: str = "Rig Builder — Code"):
    """Display a dialog with code in a scrollField for easy copying."""
    win = cmds.window(title=title, widthHeight=(450, 140), sizeable=True, tlb=True)
    form = cmds.formLayout()
    scroll = cmds.scrollField(text=code, editable=False, wordWrap=False)
    closeBtn = cmds.button(label="Close", height=28, command=lambda *_: cmds.deleteUI(win, window=True))

    cmds.formLayout(
        form, edit=True,
        attachForm=[
            (scroll, "top", 6), (scroll, "left", 6), (scroll, "right", 6),
            (closeBtn, "left", 6), (closeBtn, "right", 6), (closeBtn, "bottom", 6)
        ],
        attachControl=[(scroll, "bottom", 6, closeBtn)],
